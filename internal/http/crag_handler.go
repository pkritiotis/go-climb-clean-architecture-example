...
func NewCragHandler(app crag.UseCases) *CragHandler {
...
func (c CragHandler) GetCrag(w http.ResponseWriter, r *http.Request) {
	...
	crag, err := c.app.Queries.GetCragHandler.Handle(crag.GetCragQuery{CragID: uuid.MustParse(cragID)})
	if err == nil && crag == nil {
...
func (c CragHandler) AddCrag(w http.ResponseWriter, r *http.Request) {
	...
	err := c.app.Commands.AddCragHandler.Handle(crag.AddCragCommand{
		Name:    cragToAdd.Name,
		Desc:    cragToAdd.Desc,
...
func (c CragHandler) UpdateCrag(w http.ResponseWriter, r *http.Request) {
	...
	cragToUpdateCommand := crag.UpdateCragCommand{
...
func (c CragHandler) DeleteCrag(w http.ResponseWriter, r *http.Request) {
	...
	err := c.app.Commands.DeleteCragHandler.Handle(crag.DeleteCragCommand{CragID: uuid.MustParse(cragID)})
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		fmt.Fprint(w, err.Error())
	}
